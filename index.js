...
function keysAndValues(obj) {
  ...
  const keysArray = entriesArray.map(([key, value]) => key);
...
function filterObjectsByKey(arr, key) {
  ...
  const filterArray = arr.filter((obj) => obj.hasOwnProperty(key));
  return filterArray;
...
function getObjectValuesSum(obj) {
  ...
  const sum = values.reduce((val, current) => val + current, 0);
